Meteor.methods
  checkAnswer: (answer, problemId) ->
    user = Meteor.user()
    if not user
      throw new Meteor.Error 'logged-out', 'Du måste vara inloggad för att svara på luckor.'

    if not getSetting('answerSubmission', true)
      throw new Meteor.Error 'answering-disabled', 'Det går för tillfället inte att svara på luckor.'

    problem = Problems.findOne problemId

    if problem.activeTo and (problem.activeTo < Date.now())
      throw new Meteor.Error 'too-late', 'Åh nej! Du är för sent ute, det går tyvärr inte att svara på den här luckan längre.'
